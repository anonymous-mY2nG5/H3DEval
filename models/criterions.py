...
class Ranking_Loss(nn.Module):
    def __init__(self, margin=1.0, eps=0.1):
        super().__init__()
        self.margin = margin
        self.eps = eps
    
    def rk_loss(self, prediction, label):
        B = prediction.shape[0]
        pred_diff = prediction.view(B, 1) - prediction.view(1, B)
        label_diff = label.view(B, 1) - label.view(1, B)
        mask = (label_diff > self.eps).float()
        loss_value = F.relu(label_diff - pred_diff) * mask
        if mask.sum() > 0:
            return loss_value.sum() / mask.sum()
        else:
            return 0.0